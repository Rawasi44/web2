...
// (هذا الكود مطابق لبداية ملف learner_home.php)
if (!isset($_SESSION['userID']) || $_SESSION['userType'] !== 'learner') {
    header('Location: login.php');
    exit;
}

// --- 2. استقبال معرّف الكويز (الربط الأول) ---
// (يستقبل من learner_home.php)
$quizID = isset($_GET['quizID']) ? intval($_GET['quizID']) : 0;
if ($quizID == 0) {
    header('Location: learner_home.php'); // إذا لا يوجد كويز، أعده
    exit;
}

// --- 3. جلب بيانات الكويز (الموضوع + المعلم) (النقطة 12b) ---
$stmt = $conn->prepare("
    SELECT 
        T.topicName, 
        U.firstName, 
...
if (!$quizInfo) {
    header('Location: learner_home.php'); // إذا الكويز غير موجود
    exit;
}

// --- 4. جلب واختيار الأسئلة (النقطة 12c) ---
...
if ($questionCount > 0) {
    if ($questionCount <= 5) {
        $selectedQuestions = $allQuestions; // اعرضها كلها
    } else {
        shuffle($allQuestions); // خلط الأسئلة
        $selectedQuestions = array_slice($allQuestions, 0, 5); // اختر 5
    }
}
?>
<!DOCTYPE html>
...
<img src="uploads/<?php echo htmlspecialchars($quizInfo['photoFileName']); ?>" alt="Educator Photo" class="teacher">      <span><?php echo htmlspecialchars($quizInfo['firstName'] . ' ' . $quizInfo['lastName']); ?></span>
    </div>
...
<img src="uploads/<?php echo htmlspecialchars($q['questionFigureFileName']); ?>" alt="Question Figure" class="q-image">            <?php endif; ?>
